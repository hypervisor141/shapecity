...
public final class ModModel{
    ...
    public static final class UBO extends FSP.Modifier{
        ...
        protected void modify(FSP program, FSConfig.Policy policy){
            ...
            if(segments == 1){
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL", 0));

                vertex.addUniformBlock("std140", "MODEL", "mat4 models[" + instancecount + "]");
                vertex.addMainCode("mat4 model = models[gl_InstanceID];");

            }else if(segments == 2){
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL1", 0));
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL2", 1));
                ...
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL1", 0));
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL2", 1));
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL3", 2));
                program.addMeshConfig(new FSP.UniformBlockElement(policy,FSG.ELEMENT_MODEL, "MODEL4", 3));
